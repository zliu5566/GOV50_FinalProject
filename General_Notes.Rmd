...
page1 <- data.frame(treaty1_title, treaty1_document, treaty1_pres_date, 
  ...
  rename(title = treaty1_title, document = treaty1_document, 
         ...
         senate_action = treaty1_action, category = treaty1_type)
...
body_nodes2 <- url2_treaty %>%
  html_node("body") %>%
  html_children()
treaty2_names <- url2_treaty %>%
  html_nodes("body") %>%
  xml_find_all("//span[contains(@class, 'result-heading')]") %>%
  html_text()
treaty2_values_all <- url2_treaty %>%
  html_nodes("body") %>%
  xml_find_all("//span[contains(@class, 'result-item')]") %>%
  html_text()
...
page2 <- data.frame() %>%
  rename(title, document, date_pres, date, senate_action, category)
...
body_nodes3 <- url3_treaty %>%
  html_node("body") %>%
  html_children()
treaty3_names <- url3_treaty %>%
  html_nodes("body") %>%
  xml_find_all("//span[contains(@class, 'result-heading')]") %>%
  html_text()
treaty3_values_all <- url3_treaty %>%
  html_nodes("body") %>%
  xml_find_all("//span[contains(@class, 'result-item')]") %>%
  html_text()
...
body_nodes4 <- url4_treaty %>%
  html_node("body") %>%
  html_children()
treaty4_names <- url4_treaty %>%
  html_nodes("body") %>%
  xml_find_all("//span[contains(@class, 'result-heading')]") %>%
  html_text()
treaty4_values_all <- url4_treaty %>%
  html_nodes("body") %>%
  xml_find_all("//span[contains(@class, 'result-item')]") %>%
  html_text()
